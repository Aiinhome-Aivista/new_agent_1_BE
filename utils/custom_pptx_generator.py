...
def add_reference_architecture_slide(slide, prs, data):
    ...
    ra_title, ra_subtitle = get_dynamic_header(data, "reference_architecture", "Reference Architecture", "Enterprise logical data patterns")
    create_slide_header(slide, ra_title, ra_subtitle)
    add_footer(slide)
...
def add_azure_landscape_architecture_slide(slide, prs, data):
    ...
    proposal_title = data.get("proposal_title", "Solution") if data else "Solution"
    ala_title_default = f"Landscape Architecture ({cloud_name} Cloud Platform)"
    ala_subtitle_default = f"{cloud_name} Native Services & Integration Topology"
    ala_title, ala_subtitle = get_dynamic_header(data, "landscape_architecture", ala_title_default, ala_subtitle_default)
    create_slide_header(slide, ala_title, ala_subtitle)
    add_footer(slide)
...
def get_dynamic_header(data, slide_key, default_title, default_subtitle):
    headers = data.get("slide_headers", {}) if data else {}
    slide_header = headers.get(slide_key, {})
    title = slide_header.get("title", default_title)
...
def generate_custom_pptx(data, output_path, template_path=None, template_type="default"):
    ...
    # ----------------------------------------------------
    slide = prs.slides.add_slide(blank_slide_layout)
    bs_title, bs_subtitle = get_dynamic_header(data, "business_summary", "Business Summary", "Executive overview of the proposed solution")
    ...
    # ----------------------------------------------------
    slide = prs.slides.add_slide(blank_slide_layout)
    cr_title, cr_subtitle = get_dynamic_header(data, "client_requirements", "Client Requirements & Gap Analysis", "RAG-driven competence matching against RFP requirements")
    ...
    # ----------------------------------------------------
    slide = prs.slides.add_slide(blank_slide_layout)
    cg_title, cg_subtitle = get_dynamic_header(data, "capability_gaps", "Capability Gaps & Mitigations", "Identified gaps against RFP requirements and proposed mitigations")
    create_slide_header(slide, cg_title, cg_subtitle)
    add_footer(slide)
    ...
    # ----------------------------------------------------
    slide = prs.slides.add_slide(blank_slide_layout)
    cgc_title, cgc_subtitle = get_dynamic_header(data, "capability_gaps_cont", "Capability Gaps & Mitigations", "Identified gaps against RFP requirements and proposed mitigations")
    ...
    # ----------------------------------------------------
    slide = prs.slides.add_slide(blank_slide_layout)
    sa_title, sa_subtitle = get_dynamic_header(data, "solution_approach", "Solution Approach & Architecture", "High-level implementation strategy and operational frameworks")
    create_slide_header(slide, sa_title, sa_subtitle)
    add_footer(slide)
    ...
    hld_title, _ = get_dynamic_header(data, "high_level_design", "High Level Design: Data Flow", "")
    p_title.text = hld_title
    ...
    # ----------------------------------------------------
    slide = prs.slides.add_slide(blank_slide_layout)
    la_title, la_subtitle = get_dynamic_header(data, "architecture_layers", "Landscape & Architecture", "Reference systems architecture and integration patterns")
    create_slide_header(slide, la_title, la_subtitle)
    add_footer(slide)
    ...
    # ----------------------------------------------------
    slide = prs.slides.add_slide(blank_slide_layout)
    ia_title, ia_subtitle = get_dynamic_header(data, "infrastructure", "Infrastructure Approximation", "Estimated cloud infrastructure components and costs")
    create_slide_header(slide, ia_title, ia_subtitle)
    add_footer(slide)
    ...
    # ----------------------------------------------------
    slide = prs.slides.add_slide(blank_slide_layout)
    pm_title, pm_subtitle = get_dynamic_header(data, "project_milestones", "Project Milestones", "Sequential delivery phases and target deliverables")
    create_slide_header(slide, pm_title, pm_subtitle)
    add_footer(slide)
    ...
    # ----------------------------------------------------
    slide = prs.slides.add_slide(blank_slide_layout)
    si_title, si_subtitle = get_dynamic_header(data, "skills_inventory", "Skills Inventory & Competency Mapping", "Required technical capabilities grounded in organizational assets")
    create_slide_header(slide, si_title, si_subtitle)
    add_footer(slide)
